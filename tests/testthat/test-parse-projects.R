...
mock_project_dataframe <- function() {
  dplyr::arrange(data_frame(
...
test_that("Binding projects data frame works", {
  data <- mock_project_nodes()
  expected <- mock_project_dataframe()

  result <- dplyr::arrange(parse_projects(data), project_id)
  
  expect_identical(result, expected)
})

test_that("Tidying projects works", {
  result <- tidy_projects(mock_project_dataframe())
  expected <- data_frame(
    project_id = c("OepwndhgNNE", "py39KH8utgc"), 
    name = c("Have shared Opening Balance codebase with KPMG", "Skills to Learn SM"),
